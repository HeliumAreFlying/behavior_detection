...
| 5. 行为训练 | `train_behavior.py` | sequences（按 `split` 划分 train/val）或 grid | `checkpoints/behavior/best.pt`（best 按 7类(P+R+AP)均值+阈值搜索选取） |
...
模型结构：**双向 LSTM + 自注意力**（可用 `--no-bidirectional --no-attention` 禁用以兼容旧版）。best.pt 按 **7类(P+R+AP)均值** 选取：每轮在阈值 0.15~0.85 内搜索（F1≥0.25 约束下最大化该均值），正负样本都参与验证；每轮打印 `(P+R+AP)` 与 `thresh`。`--patience 50` 早停。
...
默认**仅评估验证集**（`--split val`），且**复用训练 val 流程**（同一 DataLoader），正负样本都评，指标准确一致。阈值在 **0.15~0.85** 内按 7类(P+R+AP)均值 最大搜索（F1≥0.25 约束）；输出表格（P/R/mAP，每类 + all）。
